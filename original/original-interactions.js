(function () {
  ...
  if (menuBtn) {
    ...
    menuBtn.addEventListener('click', () => {
      ...
      menuBtn.style.color = 'var(--ls-ink)';
      menuBtn.style.borderColor = 'var(--ls-line)';
    });
  }

  qsa('.ls-cmd-btn, section button').forEach((btn) => {
    if (btn.type === 'submit') return;
    if (btn.textContent?.includes('Start for free')) {
      btn.addEventListener('click', () => {
        const input = qs('#url-input-field');
        if (input) {
